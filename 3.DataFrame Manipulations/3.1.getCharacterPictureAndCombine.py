...
today = datetime.now()
# today = datetime(2022, 9, 9)
resultDataFrameAll = pandas.read_json("../data/characters.json")
# get characters that have birthday as today, and sorted by favourites descending order
# Sort : https://sparkbyexamples.com/pandas/pandas-sort-by-column-values-in-dataframe/
resultDataFrame = resultDataFrameAll.sort_values('favourites', ascending= False)[resultDataFrameAll.dateOfBirth == {'month':today.month, 'day': today.day}]

all_images = []
max_items = 25
i = 0

for index, character in resultDataFrame.iterrows():
